...
def pytorch2torchscript(model: nn.Module, input_shape: tuple, output_file: str,
    ...
    with torch.no_grad():
        print("Tensor device:", img_list[0].device)
        print("Model device:", next(model.parameters()).device)
        model.to('cuda')
        print("Model device:", next(model.parameters()).device)
        # print((img_list[0]).shape)
        # img_list[0] = torch.transpose(img_list[0], 1, 3)
        print((img_list[0]).shape)
...
if __name__ == '__main__':
    ...
    classifier = build_detector(cfg.model, train_cfg=None, test_cfg=cfg.test_cfg)
